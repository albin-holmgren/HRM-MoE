...
def parse_args():
    ...
    ap.add_argument('--valid-records',type=int,default=0,help='0 evaluates the whole valid split; a positive value bounds each evaluation to that many records, taken in file order')
...
def main():
    ...
    from nord_pilot.data_tools.streaming import JsonlCorpus
    corpus = JsonlCorpus({name: a.data_dir/f'{name}.jsonl' for name in ('train','valid')},
                         tok, cfg['max_seq_len'], vocab_size, a.seed)
    valid_total = corpus.count('valid')
    if a.valid_records and a.valid_records < valid_total:
        valid_total = a.valid_records
    ...
    def batch_at(name, start, budget, max_records=None):
        ...
        count = 0
        limit=corpus.count(name) if max_records is None else min(max_records,corpus.count(name))
        while count < limit:
            p, r = corpus.record(name, start+count)
            if a.stress_context:
