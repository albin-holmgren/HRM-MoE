...
class PilotTests(unittest.TestCase):
    ...
    def test_fresh_process_resume_and_export(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            run(['--out',str(td/'initialized'),'--steps','2','--schedule-steps','4','--init-export',str(td/'full/export.pt')])
            initialized=json.loads((td/'initialized/manifest.json').read_text())
            self.assertEqual(initialized['initialization']['kind'],'weights_only_export')
            self.assertEqual(initialized['initialization']['parent_step'],12)
            run(['--out',str(td/'initialized'),'--steps','4','--schedule-steps','4','--init-export',str(td/'full/export.pt'),'--resume',str(td/'initialized/latest.pt')])
